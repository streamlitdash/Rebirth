...
def test_refresh_uses_one_checker_date_for_checker_portfolios_and_risk_plan() -> None:
    ...
    manager = RiskRefreshManager(
        ...
        # 23:30 UTC on Sunday is already 00:30 Monday in Europe/London.
        clock=lambda: datetime(2026, 7, 19, 23, 30, tzinfo=timezone.utc),
    ...
    expected_checker_date = pd.Timestamp("2026-07-17")
    assert snapshot.errors == ()
    assert snapshot.revision == 1
    assert snapshot.system_date == pd.Timestamp("2026-07-20")
    assert snapshot.checker_date == expected_checker_date
    assert snapshot.market_status == "Live"
    assert market_status_calls == [pd.Timestamp("2026-07-20")]
    ...
    assert open_calls == current_calls
    assert all(call[1] == pd.Timestamp("2026-07-20") for call in open_calls)
    ...
    assert open_calls[first_open_count:] == current_calls[first_open_count:]
